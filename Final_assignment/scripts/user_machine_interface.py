...
from my_service.srv import Finalassignment
...
def main():
    rospy.init_node('user machine interface')
    ...
    pub = rospy.Publisher('/cmd_velocity', Twist, queue_size = 1)
    ...
    while not rospy.is_shutdown():
        ...
        print("""\n  
                    ...
                    (1) Robot random movements in given coordinates
                    (2) user input to move in given coordinates          
                    (3) Robot moves along the wall                   
                    (4) Robot stops ends in last position              
                    """)


        x = int(raw_input("\nclick on the robot  interface: "))
	...
	if (x == 1):
	    ...
	    resp = random_index_service(1,6)
            ...
            print("\ Target new: (" + str(random_targets[rand_index -1][0]) + ", " + str(random_targets[rand_index -1][1]) + ")")
	    ...
	    print('\nRobot is on  the  path target position.')
	    sleep(15)
            target_reached_status = 0
	    ...
	    user_input = int(raw_input("\nuser input selection in given coordinates: "))
            print("\n  target new position is ("+ str(random_targets[user_input-1][0]) + ", " + str(random_targets[user_input-1][1]) + ")")
	    ...
	    print('\nRobot is on its way for target position.')
	    sleep(15)
	    target_reached_status = 0
	    ...
	    while(target_reached_status == 0):
            ...
            print('\nRobot is moving along  the wall.')
